...
[HttpGet]
[Route("/api/weeklyweatherbyid/{id?}")]
...
[HttpGet]
[Route("/api/currentweatherbycoord/{latitude?}/{longitude?}")]
public async Task<IActionResult> getCurrentWeatherByLatLon(double latitude, double longitude) {
WeatherCoordinate currentweather = null;
using(var httpClient = new HttpClient()) {
try{
...
var stringResult = await response.Content.ReadAsStringAsync();
var rawData =  JsonConvert.DeserializeObject<WeatherCoordinate>(stringResult);
currentweather = new WeatherCoordinate{
    weather = rawData.weather.Select(element => {element.icon = "http://openweathermap.org/img/w/"+ element.icon+".png"; return element;}).ToList(),
    main = rawData.main,
    name = rawData.name,
    sys = rawData.sys,
